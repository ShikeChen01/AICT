/**
 * Agent Build Page — two tabs: Agent Builder and Dashboard.
 *
 * "Agent Builder" — full prompt block editor, model config, context budget, tools.
 * "Dashboard" — live overview of all agents: model, block count, status, templates.
 */
...
type AgentBuildTab = 'agent-builder' | 'dashboard';

const TABS: { key: AgentBuildTab; label: string; icon: React.ReactNode }[] = [
  { key: 'agent-builder', label: 'Agent Builder', icon: <Blocks className="w-4 h-4" /> },
...
export function AgentBuildPage() {
  ...
  const [activeTab, setActiveTab] = useState<AgentBuildTab>('agent-builder');
  ...
  return (
    <AppLayout>
      <div className="flex flex-1 flex-col min-h-0 overflow-hidden bg-[var(--app-bg)]">
        {/* Compact tab bar — no big heading, max vertical space for content */}
        <div className="shrink-0 border-b border-[var(--border-color)] bg-[var(--surface-card)] px-4">
          <nav className="flex items-center gap-1 h-9">
            {TABS.map((tab) => (
              <button
                key={tab.key}
                type="button"
            ...
            <span className="ml-2 text-xs text-[var(--text-faint)] truncate">{project.name}</span>
          </nav>
        </div>

        {/* Tab content */}
        <div className="flex-1 min-h-0 overflow-hidden">
          {activeTab === 'agent-builder' && projectId && (
            <PromptBuilderPage projectId={projectId} />
          )}

          {activeTab === 'dashboard' && projectId && (
            <div className="h-full overflow-y-auto">
              <main className="max-w-5xl mx-auto px-6 py-6">
                <DashboardView
                  projectId={projectId}
                  projectName={project.name}
                  onSwitchToBuilder={() => setActiveTab('agent-builder')}
                />
              </main>
            </div>
          )}
...
// Agent enriched with block data for dashboard display
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Fetch blocks for each agent in parallel
    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        {agentOverviews.length === 0 ? (
          <div className="rounded-xl border border-dashed border-[var(--border-color)] bg-[var(--surface-muted)] p-8 text-center">
            <Bot className="w-8 h-8 mx-auto text-[var(--text-faint)] mb-2" />
                ...
                >
                  ...
                  <div className="flex items-center gap-2.5 mb-3">
                    ...
                    {agent.thinking_enabled && (
                      <BrainCircuit className="w-4 h-4 text-[var(--color-accent)] shrink-0" title="Thinking enabled" />
                    )}
                  </div>

                  {/* Model pill */}
                  <div className="flex items-center gap-1.5 mb-3">
                    <Cpu className="w-3 h-3 text-[var(--text-faint)]" />
      ...
      <section>
        ...
        <div className="rounded-xl border border-[var(--border-color)] bg-[var(--surface-card)] overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[1fr_120px_140px_80px_80px] gap-2 px-4 py-2 text-[10px] font-semibold text-[var(--text-faint)] uppercase tracking-wider border-b border-[var(--border-color-subtle)] bg-[var(--surface-muted)]">
            <span>Template</span>
            <span>Role</span>
            <span>Model</span>
            <span>Thinking</span>
          ...
          {/* Divider if both system and custom */}
          {systemTemplates.length > 0 && customTemplates.length > 0 && (
            <div className="border-t border-dashed border-[var(--border-color-subtle)]" />
          )}
      ...
      <div
        ...
        role="button"
        tabIndex={0}
        onKeyDown={(e) => e.key === 'Enter' && onSwitchToBuilder()}
        className="rounded-xl border border-[var(--color-primary)]/20 bg-[var(--color-primary)]/5 p-4 flex items-center gap-3 cursor-pointer hover:bg-[var(--color-primary)]/10 transition-colors"
      >
        ...
        <ChevronRight className="w-5 h-5 text-[var(--color-primary)] shrink-0" />
      </div>
    </div>
  );
}
...
function TemplateRow({ template }: { template: AgentTemplate }) {
  ...
  return (
    <div className="grid grid-cols-[1fr_120px_140px_80px_80px] gap-2 px-4 py-2.5 items-center border-b border-[var(--border-color-subtle)] last:border-b-0 hover:bg-[var(--surface-hover)] transition-colors text-sm">
      ...
      <span className="text-xs text-[var(--text-muted)] capitalize">{template.base_role}</span>
      <span className="text-xs font-mono text-[var(--text-secondary)] truncate">{template.model}</span>
      <span>

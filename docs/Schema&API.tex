...
CREATE TABLE agents (
  ...
  role            VARCHAR(50) NOT NULL,  -- 'manager', 'engineer'
  level           VARCHAR(10),           -- NULL for Manager; 'L1' or 'L2' for engineers
  display_name    VARCHAR(100) NOT NULL, -- e.g. 'Manager', 'Engineer-3'
  ...
  sandbox_persist BOOLEAN DEFAULT FALSE, -- TRUE for Manager, FALSE for engineers
  priority        INT DEFAULT 2,         -- 0=Manager, 1=Engineer
  created_at      TIMESTAMPTZ DEFAULT NOW(),
  updated_at      TIMESTAMPTZ DEFAULT NOW()
);
\end{lstlisting}
...
Client $\leftrightarrow$ Manager conversation.
...
\subsection{Chat (Client $\leftrightarrow$ Manager)}
...
POST   & /chat/messages        & Send message to Manager (triggers Manager agent) \\
GET    & /chat/status          & Get Manager status (available/busy) \\
\bottomrule
\end{longtable}
...
\begin{lstlisting}
// Manager chat response (streamed)
{
  "type": "chat_message",
  "role": "manager",
...
// Manager status change
{
  "type": "manager_status",
  "status": "busy",
  "busy_with": "talking to Engineer-1"
}
...
\begin{lstlisting}
{
  "to_agent_role": "manager",       // or specific agent_id
...
PUT    & /tasks/:id/assign          & Assign task to agent (Manager only) \\
...
\begin{itemize}
  ...
  \item Manager: has access to sandbox and codebase; can write to spec files, task definitions, and project code (e.g.\ for review, fixes, merging)
\end{itemize}

\subsection{Spec Folder Operations (Manager)}
...
\begin{enumerate}
  \item Sandbox created on project initialization (Manager) or when first needed.
...
\begin{enumerate}
  ...
  \item Manager reviews PR:
    \begin{itemize}
      \item If approved: Manager merges PR, task status $\to$ \texttt{done}.
      \item If rejected: Manager sends ticket with feedback, engineer continues.
    \end{itemize}

...
**Legacy cloud DB stuck at `024_rag_knowledge_base`**: Run `python -m backend.scripts.upgrade_db` (used by the cloud migrate script). It performs the one-time 024-029 legacy transition to `001_baseline` before continuing with normal Alembic upgrades.
